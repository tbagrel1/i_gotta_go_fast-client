    ...
    while not JetonQuitter:
        ...
        if TempsRestant <= 0.0:
            try:
                ...
                pass
        try:
            TxtBoxTimer.configure(text=str(int(TempsRestant // 60)) + " : " +
                                  str(round(TempsRestant % 60, 1)))
            sleep(0.1)
        except:
            pass
    return
...
def ValiderSaisie(*event):
    ...
    MotTape = ""
    MotTapeB = (MotTapeBrut.get()).strip()
    for i in range(len(MotTapeB)):
        if MotTapeB[i] != " ":
            MotTape = MotTape + MotTapeB[i]
    ChaineDeValidation = []
    TempsSuivant = time()
    ValidationGenerale = False
    if TempsValidation and not JetonPause and MotTape != "" and\
...
def NouveauMot():
    ...
    if PositionBarre == -1:
        ...
        MotAttenduB = MotAttenduBrut.strip()
        for i in range(len(MotAttenduB)):
            if MotAttenduB[i] != " ":
                MotAttendu = MotAttendu + MotAttenduB[i]
        MotAttendu = MotAttendu.lower()
...
Master.withdraw()
while ValidationListeMots is False:
    NomListeMotsBrut = askopenfilename(title="Choisissez la liste de mots :",
                                       initialdir=path.dirname(sys.argv[0]),
                                       initialfile="ListeMotsDefaut.txt",
                                       filetypes=[("Fichiers Textes", "*.txt"),
                                                  ("Tous Types", "*")],
                                       defaultextension=".txt",
                                       multiple=False,
                                       parent=Master)
    if NomListeMotsBrut is None or NomListeMotsBrut == "":
        ValidationListeMots = False
    else:
        ValidationListeMots = True
Master.deiconify()

# Ouverture de la liste de mots
with open(NomListeMotsBrut, "r", encoding="iso-8859-1") as ListeMotsBrut:
...
TxtBoxSaisie = Label(SFenetreTemps,
                     text="Combien de temps voulez-vous jouer ?")
...
FenetreCompteur = Frame(Master, cursor="circle", relief="raised",
                        borderwidth=1)
...
TxtBoxSaisie.grid(row=0, column=0, columnspan=2, sticky=W+E+N+S, padx=10,
                  pady=15)
TxtBoxMot = Label(FenetreSaisie, text="", font=Police2, bg="yellow", width=40)
...
NoteBas = Label(FenetreEtat, text="Fast Writing V2.2.0 Alpha - Thomas BAGREL \
- 2015 / 2016 - Edition Test pour les Professeurs (PREVIEW)")
